...
async function submitReservation() {
    ...
    // ✅ Retrieve selections safely
    const selections = JSON.parse(localStorage.getItem('selections')) || {};
    console.log("📌 Selections:", selections);

    const rateId = selections?.rate?.rateId || null;
    const addonIds = selections?.addons?.addonIds || [];
    const extraPax = selections?.reservation?.extraPax ? parseInt(selections.reservation.extraPax) : 0;
    const extraPaxPrice = selections?.reservation?.extraPaxPrice ? parseFloat(selections.reservation.extraPaxPrice) : 0;
    ...
    console.log("🚀 Fetching rate price for rate ID:", rateId);
    let ratePrice = 0;
    try {
        const rateData = await fetchDataFromServer(rateId, 'rate');
        if (rateData && rateData.price) {
            ratePrice = parseFloat(rateData.price);
        } else {
            console.error("❌ Error: Could not retrieve rate price from server.");
            alert("Error retrieving rate price. Please try again.");
            submitButton.innerHTML = 'Submit';
            submitButton.disabled = false;
            return;
        }
        ...
        alert("Error retrieving rate price. Please check your connection.");
        submitButton.innerHTML = 'Submit';
        submitButton.disabled = false;
        return;
    }

    console.log("✅ Rate Price Retrieved:", ratePrice);

    let finalTotalPrice = ratePrice + extraPaxPrice;
    const addonsData = addonIds.length > 0 ? await fetchDataFromServer(addonIds, 'addons') : [];
    addonsData.forEach(addon => {
        finalTotalPrice += parseFloat(addon.price) || 0;
    });
    ...
    generateReservationCode().then(reservationCode => {
        ...
        formData.append('total_price', finalTotalPrice);
        formData.append('downpayment', finalDownpayment);
        formData.append('payment_receipt', paymentReceipt);
        formData.append('rate_id', rateId);
        formData.append('rate_price', ratePrice);
        ...
        formData.append('extra_pax', extraPax);
        formData.append('extra_pax_price', extraPaxPrice);
